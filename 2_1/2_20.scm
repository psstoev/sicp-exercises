...
;; (g 1 2 3 4 5 6)
;;
;; then in the body of g, w will be the list (1 2 3 4 5 6).11 Use this
;; notation to write a procedure same-parity that takes one or more
...
(define (same-parity . args)
  (define (traverse l same-par?)
    (if (null? l)
        nil
        (if (same-par? (car l))
            (cons (car l) (traverse (cdr l) same-par?))
            (traverse (cdr l) same-par?))))
  (if (null? args)
      nil
      (traverse args (if (odd? (car args)) odd? even?))))

;; Turns out Racket doesn't even have "nil"...
(define nil '())
